...
<3> Collection could also represented as property keys with `[index]` dereferencers. Like the formats in spring boot.
<4> Map types can use any key excluding the delimiter.
